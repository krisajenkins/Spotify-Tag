...
Elm.Main.make = function (_elm) {
   ...
   var step = F2(function (a,m) {
      return function () {
         switch (a.ctor)
         {case "ChooseWord":
            return _U.replace([["chosenWord"
                               ,$Maybe.Just(a._0)]],
              m);
            case "NoOp": return m;
            case "QueryResponse":
            return _U.replace([["tracks"
                               ,a._0]],
              m);}
         _U.badCase($moduleName,
   ...
   var initialModel = {_: {}
                      ,chosenWord: $Maybe.Nothing
                      ,tracks: $Http.NotAsked};
   var constructQueryUrl = function (a) {
      return A2($Basics._op["++"],
      "https://api.spotify.com/v1/search?type=track&q=",
      function () {
         switch (a.ctor)
         {case "ChooseWord": return a._0;
            case "NoOp": return "dance";}
         _U.badCase($moduleName,
         "between lines 23 and 25");
      }());
   };
   var uiMailbox = $Signal.mailbox($Schema.NoOp);
   var uiWords = A3($Signal.filter,
   function (a) {
      return function () {
         switch (a.ctor)
         {case "ChooseWord":
   ...
   $Schema.NoOp,
   uiMailbox.signal);
   var wordRequests = A2($Signal._op["<~"],
   constructQueryUrl,
   uiWords);
...
Elm.Schema.make = function (_elm) {
   ...
   var Album = F2(function (a,b) {
      return {_: {}
             ,images: b
             ,name: a};
   });
   var AlbumImage = function (a) {
      return {_: {},url: a};
   };
   var decodeAlbumImage = A2($Json$Decode.map,
   AlbumImage,
   A2($Json$Decode._op[":="],
   "url",
   $Json$Decode.string));
   var decodeAlbum = A2($Json$Decode$Extra.apply,
   A2($Json$Decode.map,
   Album,
   A2($Json$Decode._op[":="],
   "name",
   ...
   A2($Json$Decode._op[":="],
   "images",
   $Json$Decode.list(decodeAlbumImage)));
   var Artist = F2(function (a,b) {
      return {_: {},id: a,name: b};
   });
   var decodeArtist = A2($Json$Decode$Extra.apply,
   A2($Json$Decode.map,
   Artist,
   A2($Json$Decode._op[":="],
   "id",
   ...
   "name",
   $Json$Decode.string));
   var decodeTrack = A2($Json$Decode$Extra.apply,
   ...
   A2($Json$Decode._op[":="],
   "artists",
   $Json$Decode.list(decodeArtist)));
   var decodeSpotifySearch = A2($Json$Decode.at,
   _L.fromArray(["tracks"
                ,"items"]),
   $Json$Decode.list(decodeTrack));
   var QueryResponse = function (a) {
      return {ctor: "QueryResponse"
             ,_0: a};
   };
   var ChooseWord = function (a) {
      return {ctor: "ChooseWord"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   _elm.Schema.values = {_op: _op
                        ...
                        ,decodeAlbumImage: decodeAlbumImage
                        ,decodeAlbum: decodeAlbum
                        ,decodeTrack: decodeTrack
                        ,decodeSpotifySearch: decodeSpotifySearch
                        ,Model: Model};
   return _elm.Schema.values;
};
...
Elm.View.make = function (_elm) {
   ...
   var albumImage = function (album) {
      return function () {
         ...
         {case "Just":
            return A2($Html.img,
              _L.fromArray([$Html$Attributes.src(_v0._0.url)
   ...
   var wordButton = F2(function (uiChannel,
   s) {
   ...
   track) {
      ...
      track.artists)))])),
      A2($List.map,
      wordButton(uiChannel),
      A2($String.split,
      " ",
      track.name)))));
   });
   var trackListView = F2(function (uiChannel,
   tracks) {
      return A2($Html.div,
   ...
   var tracksView = F2(function (uiChannel,
   r) {
      return function () {
         switch (r.ctor)
         {case "Success":
         ...
         _L.fromArray([]),
         _L.fromArray([$Html.text($Basics.toString(r))]))]));
      }();
   });
